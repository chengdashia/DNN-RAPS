...
class Env(Correspondence):
    def __init__(self, index, ip_address, server_port, clients_list, model_name, model_cfg, batch_size):
        ...
        # 状态衍生
        self.state_buffer = []  # 添加状态缓冲区
        self.buffer_size = config.buffer_size  # 设置缓冲区大小
    ...
    def step(self, action, done):
        ...
        self.state_buffer.append(state)  # 将当前状态添加到缓冲区
        if len(self.state_buffer) > self.buffer_size:
            self.state_buffer.pop(0)  # 如果缓冲区已满,删除最早的状态
    ...
    def get_derived_state(self, state):
        if len(self.state_buffer) >= self.buffer_size:
            # 计算当前状态与历史状态之间的线性度量
            linearity_scores = [self.galois_connection(state, s) for s in self.state_buffer]
            # 选择线性度量最小的状态索引
            best_state_index = np.argmin(linearity_scores)
            # 选择具有良好线性关系的状态
            derived_state = self.state_buffer[best_state_index]
        else:
            derived_state = state

        return derived_state

    def galois_connection(self, state1, state2):
        return np.sum(np.abs(state1 - state2))  # 计算状态之间的绝对差值之和作为线性度量
    ...
    def expand_actions(self, actions, clients_list):
        full_actions = []  # 初始化完整动作列表

        for i in range(len(clients_list)):  # 遍历每个客户端
            full_actions.append(actions[self.group_labels[i]])  # 根据分组标签添加动作

        return full_actions  # 返回完整动作列表

    # 该函数将动作转换为分层信息
    def action_to_layer(self, action):
        split_layer = []  # 初始化分层列表
        for v in action:  # 遍历动作
            idx = np.where(np.abs(self.model_flops_list - v) == np.abs(self.model_flops_list - v).min())  # 找到最接近的层索引
            idx = idx[0][-1]
            if idx >= 5:  # 如果是全连接层
                idx = 6  # 将所有全连接层合并为一个选项
            split_layer.append(idx)  # 添加到分层列表
