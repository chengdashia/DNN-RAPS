...
def get_next_client(task, current_client):
    """
    获取下一个客户端
    :param task:           任务字典
...
def handle_client(conn, client_name, task, task_cumulative_layer_number, barrier):
    """
    处理客户端的请求
    :param conn:                        客户端的连接
    :param client_name:                 客户端的名称
    :param task:                        任务字典
    :param task_cumulative_layer_number: 任务的累计层数
    :param barrier:                     线程同步屏障
    :return:                            None
    """
    for iteration in range(1, max_iterations + 1):
        logging.info(f"[Thread {threading.current_thread().name}] Start processing round {iteration}")
        ...
        print(f"[Thread {threading.current_thread().name}] {client_name}: transmit time is {transmit_time} seconds.")
        # 记录客户端传输时间
        client_transmit_times[client_name].append(transmit_time)

        # 计算传输的数据量
        size = get_total_size(message)
        print(f"[Thread {threading.current_thread().name}] {client_name}: The total size of the list is {size} bytes.")
        ...
        logging.info(f"[Thread {threading.current_thread().name}] Processing time from {client_name}: {inference_time} seconds")

        # 记录客户端处理时间
        client_inference_times[client_name].append(inference_time)

        next_client_name = get_next_client(task, client_name)
        ...
        if next_client_name is None:
            ...
            logging.info(f"[Thread {threading.current_thread().name}] Total time for round {iteration}: {round_time} seconds")
            # 计算结果
            avg_loss, avg_acc = get_loss_acc(data_inference_list, target_list)
            logging.info(f"[Thread {threading.current_thread().name}] Average Loss: {avg_loss:.4f}")
            logging.info(f"[Thread {threading.current_thread().name}] Average Accuracy: {avg_acc:.4f}%")

            # 重新初始化
            logging.info(f"[Thread {threading.current_thread().name}] Round completed. Restarting with initial data.")
            data_inference_list = data_list
            processed_cumulative_layer_number = task_cumulative_layer_number
            next_client_name = list(task.keys())[0]

        next_client_conn = clients[next_client_name]
        # 序列化数据后发送
        data_to_send = [task, data_inference_list, processed_cumulative_layer_number]
...
def accept_connections(server_socket):
    ...
    """
    # 等待所有的客户端连接
    while len(clients) < len(set(client for task in node_layer_indices.values() for client in task.keys())):
    ...
    # 创建屏障来同步线程
    barrier = threading.Barrier(len(node_layer_indices))

    # 创建多线程来处理每个任务
    threads = []
    for task_name, task in node_layer_indices.items():
        first_client_name = list(task.keys())[0]
        # 消息封装
        data_to_send = [task, data_list, 0]
        # 序列化数据后发送
        ...
        task_thread = threading.Thread(
            ...
            args=(clients[first_client_name],
                  first_client_name,
                  task, 0, barrier),
            name=task_name)
        task_thread.start()
...
def main():
    ...
    server_socket.bind(('localhost', 9000))  # 将套接字绑定到 localhost 上的 9000 端口
...
if __name__ == "__main__":
    node_layer_indices = {
        'task1': {'client1': [0, 1, 2, 3], 'client2': [4, 5, 6]},
        'task2': {'client2': [0, 1, 2, 3], 'client3': [4, 5, 6]},
