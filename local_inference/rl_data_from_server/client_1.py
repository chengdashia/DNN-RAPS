...
def node_inference(data_list, node_indices, cumulative_layer_number):
    """
    开始当前节点的推理
...
def client(name, client_port=None):
    ...
    while True:
        ...
        if data:
            data_list, node_indices, cumulative_layer_number = data
            logging.info(f"{name} 收到数据: {node_indices}")
            start_time = time.time()
            processed_data_list, processed_cumulative_layer_number = node_inference(
                data_list, node_indices, cumulative_layer_number
            )

            end_time = time.time()
            process_time = end_time - start_time  # 修改计算时间的顺序
            print(f"{name} 处理数据用时: {process_time:.4f} s")
