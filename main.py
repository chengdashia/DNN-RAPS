...
def select_segmentation_points(model_cfg, num_segments):
    ...
    :param model_cfg: Configuration of the network model
...
# def choose_split_points(model, num_splits=3):
#     eligible_layers = [i for i, layer in enumerate(model.children()) if isinstance(layer, (nn.Conv3d, nn.Linear))]
#     return sorted(random.sample(eligible_layers, min(num_splits, len(eligible_layers))))
#
# # 创建模型实例
# model = VideoRecognitionCNN()
# # 随机选择分割点
# split_points = choose_split_points(model)
